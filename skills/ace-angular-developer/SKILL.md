---
name: ace-angular-developer
description: Generates Angular code and provides architectural guidance. Trigger when creating projects, components, or services, or for best practices on reactivity (signals, linkedSignal, resource), forms, dependency injection, routing, testing, or CLI tooling.
license: MIT
metadata:
  version: "1.2.0"
  authors:
...
When updating an existing component, preserve its current metadata unless the task explicitly asks for a metadata change. In Angular 18, if a component does not declare `changeDetection`, keep it that way so Angular continues using the default strategy.
...
- **RxJS Subscription Cleanup**: `takeUntilDestroyed`, `DestroyRef`, and migrating from deprecated `componentDestroyed()`. Read [rxjs-cleanup.md](references/rxjs-cleanup.md)
...
- **Fundamentals**: Best practices for unit testing (Jest), async patterns, and `TestBed`. Read [testing-fundamentals.md](references/testing-fundamentals.md)
...
- **Component Testing**: Use the ace-cypress-component-test skill
