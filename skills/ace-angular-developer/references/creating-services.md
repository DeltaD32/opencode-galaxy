...
When not to add:
- **state**: When a service has state it might be best to not add `providedIn: 'root'` to avoid state bleeding over in multiple parts of the application. Adding it is then only beneficial if the state is global.
...
## Architecture
- **Single Responsibility**: Stick to the Single Responsibility Principle. If a service has too much responsibility or gets too big, split it up.
- **Separation of Concerns** Avoid mixing API calls (data fetching) and state or business logic. Make separate services for data fetching and state.
