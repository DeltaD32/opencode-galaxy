...
This example opts into `ChangeDetectionStrategy.OnPush` explicitly. Do not treat that as a general requirement: when updating an existing Angular component that omits `changeDetection`, preserve that omission unless the task asks you to change it.
...
export class App {
  ...
  bookingForm = form(this.model, (s) => {
    ...
    validate(s.tripDetails.launchDate, ({value}) => {
      ...
      if (date < today) {
        return {kind: 'pastDate', message: 'Launch date must be in the future'};
      }
      return undefined;
    });
