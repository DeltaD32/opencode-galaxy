...
def _open_nav_burger(page: Page) -> bool:
    """
    Open the APEX left-nav side drawer (hamburger button) so TreeNav links
    become visible in the DOM before we try to read them.

    APEX collapses the TreeNav on small viewports and some page templates.
    The hamburger button (#t_Button_navControl) toggles aria-expanded on
    #t_TreeNav. We click it if the nav is currently collapsed.

    Returns True if the nav was successfully opened (or was already open).
    """
    try:
        # Check if TreeNav is already expanded / visible
        nav_visible = page.evaluate("""() => {
            const nav = document.querySelector('#t_TreeNav, .t-Body-nav, .t-TreeNav');
            if (!nav) return false;
            const r = nav.getBoundingClientRect();
            const bodyHasCollapsed = document.body.classList.contains('js-navCollapsed');
            return r.width > 0 && !bodyHasCollapsed;
        }""")
        ...
        burger = page.locator('#t_Button_navControl').first
        if burger.count() == 0:
            burger = page.locator('[aria-controls="t_TreeNav"]').first
        if burger.count() == 0:
            return False  # no burger found

        burger.click()
        time.sleep(0.8)  # wait for slide-in animation

        # Verify it opened
        return page.evaluate("""() => {
            const nav = document.querySelector('#t_TreeNav, .t-Body-nav');
            if (!nav) return false;
            return !document.body.classList.contains('js-navCollapsed');
        }""")

    except Exception as e:
        print(f"  ⚠️  _open_nav_burger: {e}")
        return False


def _nav_to_page_by_name(page: Page, page_name: str, apex_host: str) -> bool:
    """
    Navigate to a page by matching its name in the TreeNav — never via direct
    URL. Opens the burger, finds the link whose text contains page_name
    (case-insensitive), clicks it, and waits for the new page to load.

    This avoids SSO/session redirect errors that occur when navigating directly
    to a URL whose session token has expired or differs from the active one.

    Returns True if navigation succeeded, False otherwise.
    """
    print(f"  🍔 Opening burger nav to find '{page_name}'...")
    _open_nav_burger(page)
    time.sleep(0.4)

    # Find a matching link in the nav
    match = page.evaluate(f"""() => {{
        const needle = {json.dumps(page_name.lower())};
        const anchors = Array.from(document.querySelectorAll(
            '#t_TreeNav a[href], .t-TreeNav a[href], .t-Body-nav a[href], '
            + '.a-TreeView-content a[href]'
        ));
        for (const a of anchors) {{
            const text = (a.textContent || '').trim().toLowerCase();
            const href = a.href || '';
            // Match by text OR by URL path segment
            if (text.includes(needle) || href.toLowerCase().includes(needle)) {{
    ...
    if not match:
        print(f"  ⚠️  No nav link found matching '{page_name}'")
        return False

    print(f"  ✅ Found nav link: '{match['text']}' → {match['href'][:60]}")

    # Click the link — in-app navigation, no direct goto
    try:
        link_el = page.locator(f"#t_TreeNav a[href*='{page_name.lower()}'], "
                               f".t-TreeNav a[href*='{page_name.lower()}'], "
                               f".t-Body-nav a[href*='{page_name.lower()}']").first
        if link_el.count() == 0:
            # Fall back to text match
            link_el = page.get_by_role("link", name=re.compile(page_name, re.IGNORECASE)).first

        link_el.click()
        # Wait for the page to land on the APEX host (not SSO)
        ...
        while time.time() < deadline:
            ...
            if apex_host in page.url:
                break
        time.sleep(1.5)
        return apex_host in page.url
    except Exception as e:
        print(f"  ⚠️  Click failed for '{page_name}': {e}")
        return False


def _enumerate_nav_links(page: Page) -> list[dict]:
    """
    Return all navigable links from the left TreeNav / side nav.

    Opens the nav burger first so collapsed nav items become visible,
    then reads every <a href> inside the nav tree. Deduplicates by href.
    Excludes javascript:, #action$, and anchor-only links.
    """
    # Open the side nav so all links are visible in the DOM
    _open_nav_burger(page)
    time.sleep(0.3)

    return page.evaluate("""() => {
        // Collect from all possible nav containers (TreeNav, side nav, nav bar)
        const selectors = [
            '#t_TreeNav a[href]',
            '.t-TreeNav a[href]',
            '.t-Body-nav a[href]',
            '.a-TreeView-content a[href]',
            '.a-TreeView-label[href]',
            '.t-Nav-link[href]',
        ];
        const seen  = new Set();
        const links = [];
        selectors.forEach(sel => {
            document.querySelectorAll(sel).forEach(a => {
                const href = (a.href || '').split('?')[0];  // dedupe ignoring session params
                const raw  = a.href || '';
                if (!raw) return;
                if (raw.startsWith('javascript')) return;
                if (raw.includes('#action$')) return;
...
def run_audit(
    ...
    with sync_playwright() as pw:
        ...
        # Collect the full nav link list once up front (burger is opened by
        # _enumerate_nav_links so all links are visible regardless of template).
        # We collect ALL hrefs first so we don't need to go back to home between
        # pages — just navigate directly to each URL in sequence.
        ...
        for link in nav_links:
            ...
            # Navigate via the burger nav (click the link) — never direct goto.
            # Direct URL navigation risks SSO session redirects when the URL
            # contains a stale session token or the page slug differs.
            if not _nav_to_page_by_name(page, text, apex_host):
                # Fallback: try the URL path segment (last part of href)
                slug = href.rstrip('/').split('/')[-1]
                if not _nav_to_page_by_name(page, slug, apex_host):
                    report.add("info", "NAV-SKIPPED",
                               f"Could not navigate to '{text}' via burger nav — skipping",
                               surface=safe)
                    continue
            ...
            # Dialogs triggered from this page
            _audit_dialogs(page, report, shots_dir, injected_css, safe)

            # No need to go back to home — we already have the full nav_links list.
            # Re-inject CSS on whatever page we're on now (dialogs may have navigated).
            for label, css in injected_css.items():
                _inject_css(page, css, label)
