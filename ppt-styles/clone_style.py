...
# ── constants ─────────────────────────────────────────────────────────────────
REGISTRY_DIR  = Path.home() / ".config" / "opencode" / "ppt-styles"
...
def generate_brand_guide_md(style: dict, name: str, source_file: str,
    ...
    major_font    = fonts.get("major_latin", "BMWGroupTN Condensed")
    minor_font    = fonts.get("minor_latin", "BMWGroupTN Condensed")
    dark_layouts  = [l for l in layouts if l.get("is_dark") is True]
    light_layouts = [l for l in layouts if l.get("is_dark") is False]

    # BMW Group font family detection
    BMW_FONT_PREFIXES = ("BMWGroupTN", "BMW Group", "BMWGroup")
    major_is_bmw = any(major_font.startswith(p) for p in BMW_FONT_PREFIXES)
    minor_is_bmw = any(minor_font.startswith(p) for p in BMW_FONT_PREFIXES)
    ...
    # ── CI Anchor section (BMW CI gets a rules section; clones get an inheritance note) ──
    if is_builtin:
        lines += [
            f"## BMW Group Corporate Identity Rules",
            f"",
            f"This is the **reference style**. All other registered styles are clones that inherit",
            f"structural elements from this CI and override visual ones.",
            f"",
            f"### Font Family Constraint",
            f"",
            f"All BMW Group presentations **must** use a BMW Group typeface. Permitted font families:",
            f"",
            f"| Family | Variants | Use |",
            f"|--------|----------|-----|",
            f"| `BMWGroupTN Condensed` | Regular, Bold, Italic | Titles, body, UI labels (primary) |",
            f"| `BMW Group Condensed`  | Regular, Bold         | Alternative body / legacy decks   |",
            f"| `BMW Group`            | Regular, Bold, Italic | Long-form body text               |",
            f"",
            f"> **Never substitute** Arial, Calibri, or other system fonts in BMW presentations.",
            f"> If a BMW font is unavailable on the rendering machine, flag it rather than silently falling back.",
            f"",
            f"### Primary Brand Color",
            f"",
            f"- **Ocean Blue** `#035970` — the mandatory primary color for titles, headers, and key accents.",
            f"- Secondary palette: `#548D9E` (accent1), `#85ACB9` (accent2), `#92A2BD` (lt2).",
            f"- Never use red, orange, or non-BMW-palette colors for structural elements in the default CI.",
            f"",
            f"### Structural Elements Inherited by All Clones",
            f"",
            f"| Element | Spec |",
            f"|---------|------|",
            f"| Slide dimensions | 13.333\" × 7.5\" (16:9, 12192000 × 6858000 EMU) |",
            f"| Footer shape | `FußzeileAU1` at `(0.535\", 7.26\")` — update text, never reposition |",
            f"| Page number | `SeitenzahlAU1` at `(12.4\", 7.26\")` — text `- ‹#› -` |",
            f"| Divider line | `BMW Group Trennlinie` — horizontal rule above footer |",
            f"",
            f"---",
            f"",
        ]
    else:
        # Cloned style — show what it inherits vs overrides
        inherits_font_family = major_is_bmw or minor_is_bmw
        lines += [
            f"## BMW Group CI Anchor",
            f"",
            f"This style is a **clone** — it stands on its own visually but anchors to BMW Group CI",
            f"structural rules. The table below shows what is inherited vs overridden.",
            f"",
            f"| Element | Inherited from BMW CI | This style's value |",
            f"|---------|----------------------|-------------------|",
            f"| Slide dimensions | ✅ 13.333\" × 7.5\" (16:9) | same |",
            f"| Font family constraint | {'✅ BMW Group typeface' if inherits_font_family else '⚠️ non-BMW font — verify CI compliance'} | `{major_font}` |",
            f"| Footer shape names | ✅ `FußzeileAU1`, `SeitenzahlAU1`, `BMW Group Trennlinie` | same (from master) |",
            f"| Primary brand color | ❌ overridden | `{primary_color}` (extracted from this deck) |",
            f"| Dark layouts | ❌ overridden | {len(dark_layouts)} dark / {len(light_layouts)} light layouts |",
            f"| Title color | ❌ overridden | `{t.get('title_color_hex') or t.get('title_color_scheme') or '—'}` |",
            f"",
            f"### Font Family Rule",
            f"",
            f"{'This style uses BMW Group fonts — CI compliant.' if inherits_font_family else '⚠️ This style uses a non-BMW font. Verify with CI guidelines before distributing externally.'}",
            f"",
            f"Permitted BMW Group font families (for any BMW Group presentation):",
            f"- `BMWGroupTN Condensed` — primary, all weights",
            f"- `BMW Group Condensed` — alternative / legacy",
            f"- `BMW Group` — long-form body text",
            f"",
            f"**Active in this style:** `{major_font}` (headings) / `{minor_font}` (body)",
            f"",
            f"### Primary Brand Color for This Style",
            f"",
            f"| Color | Hex | Source |",
            f"|-------|-----|--------|",
            f"| Primary | `{primary_color}` | {'dark layout background fill' if dark_layouts else 'title text / dominant corpus'} |",
        ]
        # Add palette row if meaningful
        palette = [c for c in t.get("dominant_colors", []) if c != primary_color][:5]
        if palette:
            lines.append(f"| Supporting palette | {' / '.join(f'`{c}`' for c in palette)} | extracted shape fills |")
        lines += [f"", f"---", f""]
...
def bootstrap_bmw_ci() -> None:
    """Ensure the built-in bmw-ci entry exists in the registry."""
    registry = load_registry()
    if BMW_SLUG in registry["styles"]:
        return
    skill_dir   = Path.home() / ".opencode" / "skills" / "bmw-ppt-creator"
    brand_guide = skill_dir / "references" / "brand-styling-guide.md"
    registry["styles"][BMW_SLUG] = {
        "slug":         BMW_SLUG,
        "name":         "BMW CI (Default)",
        ...
        "style_dir":    str(skill_dir),
        "brand_guide_path": str(brand_guide) if brand_guide.exists() else None,
        "is_builtin":   True,
        ...
        "typography_summary": {
            ...
            "title_size_pt": 26.0,
            "body_size_pt":  18.0,
            "primary_color": "#035970",
            "dominant_color": "#035970",
            "palette":       ["#035970", "#548D9E", "#85ACB9", "#92A2BD"],
        },
        "theme_summary": {
            "scheme_name": "BMW Group 21",
            "dk1": "#000000", "lt1": "#FFFFFF",
            "dk2": "#035970", "accent1": "#548D9E",
            "major_font": "BMWGroupTN Condensed",
            "minor_font": "BMWGroupTN Condensed",
        },
        "layout_stats": {"total": 20, "dark": 0, "light": 20},
        "has_title_bg": False,
        "total_slides_in_source": None,
        "layout_count": 20,
    }
    save_registry(registry)
    print(f"[bootstrap] BMW CI registered as '{BMW_SLUG}'.")
...
def _pick_primary_color(style: dict, is_builtin_bmw_ci: bool = False) -> str | None:
    ...
    Cloned styles — priority:
      ...
      5. theme dk2 (inherited BMW Ocean Blue — last resort only)
    """
    clr = style.get("theme", {}).get("color_scheme", {})
    dk2 = clr.get("dk2", {}).get("hex")

    if is_builtin_bmw_ci:
        return dk2  # always #035970
    ...
    # 5. dk2 fallback (inherited BMW Ocean Blue)
    return dk2
...
def register_style(slug: str, name: str, description: str,
    ...
    registry["styles"][slug] = {
        ...
        "is_builtin":   is_builtin,        # True only for bmw-ci
...
def main() -> None:
    ...
    is_builtin_clone = (slug == BMW_SLUG)
